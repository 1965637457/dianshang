...
<form action="<?php echo U('update');?>" method="post" enctype="multipart/form-data" class="form-data" id="j-fmData">
    ...
    <table class="table table-condensed tb-data">
        <tr>
            ...
            <td>
                <label><input type="radio" name="status" value="1" checked /> <?php echo L('ACTIVE');?></label>
        ...
        <tr>
            ...
            <td>
                <select name="pid" class="form-control-sm">
                    ...
                    <?php if(is_array($parents)): $i = 0; $__LIST__ = $parents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if(($data["pid"]) == $v["id"]): ?>selected="selected"<?php endif; ?>><?php echo (do_sub_prefix($v["grade"])); echo ($v["title_cn"]); ?> [<?php echo ($v["name"]); ?>]</option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
                <input type="hidden" name="old_pid" value="<?php echo ($data["pid"]); ?>"/>
            </td>
        </tr>
        <tr>
            <th><?php echo L('NAME');?>[中]</th>
            <td><input type="text" name="title_cn" value="<?php echo (htmlspecialchars($data["title_cn"])); ?>" class="form-control-sm" /></td>
        </tr>
        <tr>
            <th><?php echo L('NAME');?>[英]</th>
            <td><input type="text" name="title_en" value="<?php echo (htmlspecialchars($data["title_en"])); ?>" class="form-control-sm" /></td>
        </tr>
        <tr>
            <th>权限代码</th>
            <td><input type="text" name="name" value="<?php echo (htmlspecialchars($data["name"])); ?>" class="form-control-sm" /></td>
        </tr>
    </table>
