...
<form action="<?php echo U('insert');?>" method="post" enctype="multipart/form-data" class="form-data" id="j-fmData">
    ...
    <table class="table table-condensed tb-data">
        ...
        <tr>
            ...
            <td>
                <select name="pid" class="form-control-sm">
                    ...
                    <?php if(is_array($parents)): $i = 0; $__LIST__ = $parents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if(($pid) == $v["id"]): ?>selected="selected"<?php endif; ?>><?php echo (do_sub_prefix($v["grade"])); echo ($v["category"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td><input type="text" name="category" class="form-control-sm" /></td>
        </tr>
    </table>
