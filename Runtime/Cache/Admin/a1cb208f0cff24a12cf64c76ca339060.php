...
<form action="<?php echo U('update');?>" method="post" enctype="multipart/form-data" class="form-data" id="j-fmData">
    ...
    <div class="tab-content">
            ...
            <table class="table table-condensed tb-data">
                <tr>
                    ...
                    <td>
                        ...
                        <label><input type="radio" name="display" value="0" <?php if(($data["display"]) == "0"): ?>checked<?php endif; ?> /> 图像</label>
                    </td>
                </tr>
                <tr>
                    <th><?php echo L('NAME');?></th>
                    <td><input type="text" name="spec" value="<?php echo (htmlspecialchars($data["spec"])); ?>" class="form-control-sm" /></td>
                </tr>
                <tr>
                    <th><?php echo L('ALIAS');?></th>
                    <td><input type="text" name="spec_alias" value="<?php echo (htmlspecialchars($data["spec_alias"])); ?>" class="form-control-sm" /></td>
                </tr>
                <tr>
        ...
        <div class="tab-pane" id="pane2">
            <table class="table table-condensed table-bordered tb-data-list" id="js-spec-value-list" data-add-url="<?php echo U('SpecValue/add',array('specid'=>$data['id']));?>" data-edit-url="<?php echo U('SpecValue/edit');?>" data-remove-url="<?php echo U('SpecValue/delete');?>">
                <thead>
                    <tr>
                ...
                <?php if(is_array($data["_list"])): $i = 0; $__LIST__ = $data["_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr data-id="<?php echo ($v["id"]); ?>">
                        ...
                        <td><?php if(!empty($v["spec_image"])): ?><img src="<?php echo ($v["spec_image"]); ?>" width="30" height="30"><?php endif; ?></td>
                        <td class="f16">
                            ...
                            <input type="hidden" name="spec_values[]" value="<?php echo ($v["id"]); ?>">
                        </td>
