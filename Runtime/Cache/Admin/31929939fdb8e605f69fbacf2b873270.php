...
<div class="page-direction clearfix">
    ...
    <form action="<?php echo U('search');?>" method="get" class="list-search">
        <input type="text" name="kw" value="<?php echo ($s_opts["kw"]); ?>" class="form-control-sm" />
        <button type="submit" class="btn btn-primary btn-sm" name="search"><i class="glyphicon glyphicon-search"></i> <?php echo L('SEARCH');?></button>
    </form>
    ...
    <div class="table-responsive">
        <table class="table table-condensed table-hover tb-list">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><?php echo ($v["id"]); ?></td>
                    <td><a href="<?php echo U('edit',array('id'=>$v['id']));?>"><?php echo ((isset($v["title"]) && ($v["title"] !== ""))?($v["title"]):'== 未定义 =='); ?></a></td>
