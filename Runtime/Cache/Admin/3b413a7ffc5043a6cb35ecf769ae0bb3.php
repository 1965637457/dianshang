        ...
        <table class="table table-condensed table-hover tb-list" id="tree-list">
            <thead>
                <tr>
                    ...
                    <th class="th-150"><?php echo L('CHILD_NODE');?></th>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr data-tt-id="<?php echo ($v["id"]); ?>" <?php if(($v["pid"]) > "0"): ?>data-tt-parent-id="<?php echo ($v["pid"]); ?>"<?php endif; ?>>
                ...
                <td><?php echo ($v["id"]); ?></td>
                <td><a href="<?php echo U('edit',array('id'=>$v['id']));?>"><?php echo ((isset($v["title_cn"]) && ($v["title_cn"] !== ""))?($v["title_cn"]):'== 未定义 =='); ?>|<?php echo ((isset($v["title_en"]) && ($v["title_en"] !== ""))?($v["title_en"]):'== UNDEFINED =='); ?></a><?php if(in_array(($v["grade"]), explode(',',"3,4"))): ?>[<?php echo ($v["name"]); ?>]<?php endif; ?></td>
                <td><?php if(($v["grade"]) < "4"): ?><a href="<?php echo U('add',array('pid'=>$v['id']));?>" class="btn-add"><?php echo L('ADD');?></a><?php endif; ?></td>
