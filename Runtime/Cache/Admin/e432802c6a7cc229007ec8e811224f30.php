    ...
    <div class="table-responsive">
        <table class="table table-condensed table-hover tb-list">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><?php echo ($v["id"]); ?></td>
                    <td><a href="<?php echo U('edit',array('id'=>$v['id']));?>"><?php echo ((isset($v["coupon_rule"]) && ($v["coupon_rule"] !== ""))?($v["coupon_rule"]):'== 未定义 =='); ?></a></td>
                    <td><?php echo ($v["coupon_alias"]); ?></td>
                    <td><?php echo ($coupon_types[$v['coupon_type']]); ?></td>
                    <td><?php echo ($v["min_amount"]); ?></td>
                    <td><?php if(($v["coupon_type"]) == "1"): echo ($v["discount_amount"]); else: echo ($v["discount_rate"]); ?>%<?php endif; ?></td>
                    <td><?php echo ($target_types[$v['target_type']]); ?></td>
                    <td><?php echo ($limit_types[$v['limit_type']]); ?></td>
                    <td>始<?php echo (date('Y-m-d H:i:s',$v["start_time"])); ?><br/>
                        至<?php echo (date('Y-m-d H:i:s',$v["end_time"])); ?></td>
                    <td><?php echo ($v["total_batch"]); ?></td>
                    ...
                    <td>
                        ...
                        <a href="<?php echo U('Coupon/index',array('rid'=>$v['id']));?>" class="btn btn-danger btn-xs">查看优惠券</a>
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu pull-right">
                                <li><a href="<?php echo U('grant',array('id'=>$v['id']));?>">发放优惠券</a></li>
                                <li><a href="<?php echo U('delete',array('id'=>$v['id']));?>" class="j-del"><?php echo L('DELETE');?></a></li>
                            </ul>
                        </div>
                    </td>
