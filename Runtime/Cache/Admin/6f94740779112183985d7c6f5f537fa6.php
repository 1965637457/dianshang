...
<form action="<?php echo U('update');?>" method="post" enctype="multipart/form-data" class="form-data" id="j-fmData">
    ...
    <ul class="nav nav-tabs">
        ...
        <li><a data-target="#pane2" data-toggle="tab">商品列表</a></li>
        ...
        <div class="tab-pane active" id="pane1">
            <table class="table table-condensed tb-data">
                <tr>
                    ...
                    <td>
                        <label><input type="radio" name="business[status]" value="1" checked /> <?php echo L('ACTIVE');?></label>
                        <label><input type="radio" name="business[status]" value="0" <?php if(($data["status"]) == "0"): ?>checked<?php endif; ?> /> <?php echo L('DISABLED');?></label>
                    </td>
                </tr>
    <!--             <tr>
      			...
      			<tr>
                    <th>商家名称</th>
                    <td><input type="text" name="business[business_name]" value="<?php echo ($data["business_name"]); ?>" class="form-control-sm" /> <i></i></td>
                </tr>
                <tr>
                    <th>联系人</th>
                    <td><input type="text" name="business[contact_name]"  value="<?php echo ($data["contact_name"]); ?>" class="form-control-sm" /> <i></i></td>
                </tr>
                <tr>
                    <th>tel</th>
                    <td><input type="text" name="business[mobile]"  value="<?php echo ($data["mobile"]); ?>" class="form-control-sm" /></td>
                </tr>
                <tr>
                    <th>eamil</th>
                    <td><input type="text" name="business[email]"  value="<?php echo ($data["email"]); ?>" class="form-control-sm" /></td>
                </tr>
                <tr>
                    <th>商家地址</th>
                    <td><input type="text" name="business[business_address]"  value="<?php echo ($data["business_address"]); ?>" class="form-control-sm" /></td>
                </tr>
                <tr>
                    <th><?php echo L('REMARK');?></th>
                    <td><textarea name="business[business_info]" class="form-control-sm"><?php echo ($data["business_info"]); ?></textarea></td>
                </tr>
            </table>
        </div>
        ...
        <div class="tab-pane" id="pane2">
            <table class="table table-condensed table-bordered tb-data-list" id="js-address-list" data-reload-url="<?php echo U('MemberAddress/reloadaddress',array('mid'=>$data['id']));?>" data-add-url="<?php echo U('MemberAddress/add',array('mid'=>$data['id']));?>" data-edit-url="<?php echo U('MemberAddress/edit');?>" data-remove-url="<?php echo U('MemberAddress/delete');?>">
                ...
                <tbody>
                 ...
                 <?php if(is_array($data["_business_list"])): $i = 0; $__LIST__ = $data["_business_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$b): $mod = ($i % 2 );++$i;?><tr data-id="<?php echo ($b["id"]); ?>"> 
                        <td><?php echo ($b["goods_id"]); ?></td>
                        <td><a href="<?php echo U('Goods/edit',array('id'=>$b['goods_id']));?>"><?php echo ($b["goods"]); ?></a></td>
