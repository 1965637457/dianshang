...
class Page {
    ...
    private $config = array(
        'header' => '<li><span>共 %TOTAL_ROW% 条</span></li>',
        'prev' => '<<',
        'next' => '>>',
        'first' => '1',
        'last' => '%TOTAL_PAGE%',
        'jump' => '<li><input type="text" value="%NOW_PAGE%" class="form-control-sm t-xxs"/> <button type="button" class="btn btn-sm btn-danger j-jumpPage">跳转</button></li>',
//        'theme'  => '%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%',
        'theme' => '%HEADER% %UP_PAGE% %FIRST% %LINK_PAGE% %END% %DOWN_PAGE% %JUMP%',
    );
    ...
    public function show() {
        ...
        if ($this->totalPages > $this->rollPage && ($this->nowPage - $now_cool_page_ceil) >= 1) {
            ...
            if(($this->nowPage - $now_cool_page_ceil) > 1){
        ...
        if ($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages) {
            ...
            if(($this->nowPage + $now_cool_page_ceil) < $this->totalPages){
                $the_end = '<li><span>...</span></li>'. $the_end;
            }
        }
        ...
        $page_str = str_replace(
                array('%HEADER%', '%JUMP%', '%NOW_PAGE%', '%UP_PAGE%', '%DOWN_PAGE%', '%FIRST%', '%LINK_PAGE%', '%END%', '%TOTAL_ROW%', '%TOTAL_PAGE%'), array($this->config['header'], $this->config['jump'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages), $this->config['theme']);
        return $page_str;
    }
...
//        $up_page = $up_row > 0 ? '<a class="prev" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a>' : '';
        $up_page = $up_row > 0 ? '<a href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a>' : '<a href="#none">' . $this->config['prev'] . '</a>';

        //下一页
        $down_row = $this->nowPage + 1;
//        $down_page = ($down_row <= $this->totalPages) ? '<a class="next" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a>' : '';
        $down_page = ($down_row <= $this->totalPages) ? '<a href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a>' : '<a href="#none">' . $this->config['next'] . '</a>';
...
//        }
        ...
        for ($i = 1; $i <= $this->rollPage; $i++) {
            ...
            if ($page > 0 && $page != $this->nowPage) {
                ...
                if ($page > 0 && $this->totalPages != 1) {
                    $link_page .= '<a href="#none" class="on">' . $page . '</a>';
                }
            }
        }
        $link_page .= '';
