...
class Verify {
    ...
    public function simple($id = ''){
        ...
        $this->fontttf = $ttfPath . '1.ttf';
        
        // 干扰
        for ($i = 0; $i < 12; $i++) {
            imagearc($this->_image, mt_rand(-10, $this->imageW), mt_rand(-10, $this->imageH), mt_rand(30, 300), mt_rand(20, 200), 55, 44, $this->_color);
        }
        for ($i = 0; $i < 100; $i++) {
            imagesetpixel($this->_image, mt_rand(0, $this->imageW), mt_rand(0, $this->imageH), $this->_color);
        }
        // 绘验证码
        $code = array(); // 验证码
        $codeNX = 0; // 验证码第N个字符的左边距
