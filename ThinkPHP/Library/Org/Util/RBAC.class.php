...
class RBAC {
    ...
    //检查当前操作是否需要认证
    static function checkAccess($actionName, $moduleName, $appName) {
    ...
    //权限认证的过滤器方法
    static public function AccessDecision($actionName = ACTION_NAME, $moduleName = CONTROLLER_NAME, $appName = MODULE_NAME) {
        //检查是否需要认证
        if (RBAC::checkAccess($actionName, $moduleName, $appName)) {
            //存在认证识别号，则进行进一步的访问决策
            ...
            if (!session(C('ADMIN_AUTH_KEY'))) {
                if (C('USER_AUTH_TYPE') == 2) {
                    ...
                    //通过数据库进行访问检查
                    $auth = session(C('USER_AUTH_KEY'));
                    $accessList = RBAC::getAccessList($auth['id']);
                } else {
    ...
    static public function getAccessList($authId) {
        ...
        $sql = "select node.id,node.name from " .
                ...
                $table['node'] . " as node " .
                "where user.id='{$authId}' and user.role_id=role.id and access.role_id=role.id and role.status=1 and access.node_id=node.id and node.grade=3";
        ...
        foreach ($modules as $key => $module) {
            ...
            $sql = "select node.id,node.name from " .
                    ...
                    $table['access'] . " as access ," .
                    $table['node'] . " as node " .
                    "where user.id='{$authId}' and user.role_id=role.id and access.role_id=role.id and role.status=1 and access.node_id=node.id and node.grade=4 and node.pid={$moduleId}";
            $rs = $db->query($sql);
            $action = array();
            foreach ($rs as $a) {
                $action[$a['name']] = $a['id'];
